      ...
      <div className="flex flex-col h-full">
        ...
        <div className="px-4 py-2.5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <div className="flex items-center gap-1.5">
            <button onClick={() => setShowHistory(!showHistory)} className="text-[11px] px-2.5 py-1 rounded-lg bg-white border border-slate-200 text-slate-600 hover:border-brand-200 hover:text-brand-600 transition-all">
              {showHistory ? '← Chat' : '📋 History'}
            </button>
            <button onClick={handleNewChat} className="text-[11px] px-2.5 py-1 rounded-lg bg-brand-50 border border-brand-100 text-brand-600 hover:bg-brand-100 transition-all">
              + New
            </button>
          </div>
                ...
                <div className="text-center py-10 animate-fade-in">
                  ...
                  <div className="mt-5 space-y-1.5">
                    {['How am I doing overall?', "What's my attendance?", 'Am I overspending?', 'What deadlines are coming?', 'What do I need to buy?'].map((q) => (
                      <button key={q} onClick={() => setInput(q)} className="block w-full text-left text-[12px] px-3 py-2 bg-slate-50 rounded-xl text-slate-600 hover:bg-brand-50 hover:text-brand-700 transition-all duration-150 border border-transparent hover:border-brand-100">{q}</button>
                    ))}
                  </div>
                </div>
              )}
            ...
            <form onSubmit={handleSend} className="px-4 py-3 border-t border-slate-100 bg-white">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  disabled={loading}
                  className="flex-1 px-4 py-2.5 text-[13px] bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-brand-500/30 focus:border-brand-400 disabled:opacity-50 transition-all placeholder:text-slate-400"
                />
