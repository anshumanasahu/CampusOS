/**
 * Attendance calculator — mirrors backend logic exactly.
 *
 * Rules (from PRD):
 ...
 *
 * Formula matches: backend/src/utils/attendance.js
 */

/**
 * Calculate attendance stats from records.
 * @param {Array} records - Array of { status } objects
 * @param {number} targetThreshold - Target percentage (e.g. 75)
...
export const calculateAttendanceStats = (records, targetThreshold = 75) => {
  ...
  // Safe skips: max N where attended / (total + N) >= threshold/100
  // Rearranged: N <= (attended * 100 / threshold) - total
  const safeSkips = calculateSafeSkips(attended, total, targetThreshold);

  // Classes needed: min N where (attended + N) / (total + N) >= threshold/100
